...
Читает данные из positions_export.csv
...
DOMAIN_COLORS = {
    ...
    'fedomo.ru':             '#F39C12',
    'svetodom.ru':           '#1ABC9C',
    'basicdecor.ru':         '#D35400',
    'lustranadom.ru':        '#2C3E50',
    'citilux.ru':            '#E91E63',
    'lampsshop.ru':          '#00BCD4',
    'eurosvet.ru':           '#FF5722',
    'maytoni.ru':            '#607D8B',
    'elektrostandard.ru':    '#795548',
    'msveta.ru':             '#009688',
    'lustrypremium.ru':      '#FF9800',
}
...
def is_brand(q):
    return any(p in str(q).lower() for p in BRAND_PATTERNS)

# Фильтрация
df_e = df_all[df_all['engine'] == engine_key].copy()
...
if hide_brand:
    mask = df_cur['query'].apply(is_brand)
    df_cur = df_cur[~mask]
    if not df_prev.empty:
        df_prev = df_prev[~df_prev['query'].apply(is_brand)]
...
not_found = total_q - int(our_cur['query'].nunique())
...
for col, val, prev, label in [
    (c1, f'{vis_cur:.1f}%', vis_prev, 'Видимость svetlux.ru'),
    ...
    try:
        ...
        p_num = float(str(prev).replace('%','')) if prev is not None else None
        d = delta_html(v_num, p_num)
    except:
        d = ''
    col.markdown(
...
with col_l:
    ...
    for d in show_domains:
        ...
        visp = calc_vis(dp['position'], total_q) if not dp.empty else None
        rows.append({
            'Домен': d,
            'Видимость': vis,
            'Δ': round(vis - visp, 1) if visp is not None else None,
...
with col_r:
    ...
    for dt in sorted(df_all['check_date'].unique()):
        ...
        if hide_brand:
            df_dt = df_dt[~df_dt['query'].apply(is_brand)]
...
with tab1:
    ...
    comp_data = {'Запрос': queries}
    for d in show_domains:
        pm = dict(zip(df_cur[df_cur['domain']==d]['query'],
                      df_cur[df_cur['domain']==d]['position']))
        comp_data[d] = [int(pm[q]) if q in pm and pd.notna(pm[q]) else None for q in queries]
...
st.divider()
st.caption(f'Данные: positions_export.csv · Источник: XMLRiver.Parser · Топ-50 · Москва')
